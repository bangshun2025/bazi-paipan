...
(function() {
  ...
  (function() {
    ...
    function checkBtn(container, label) {
      ...
      var lvlWrap = b.closest('.cmp-level-wrap');
      var nx = lvlWrap ? lvlWrap.nextElementSibling : b.nextElementSibling;
  ...
  // ===== 盘面对比(v0.37.0) =====
  tests.push({ section:'盘面对比(v0.37.0)' });
  (function() {
    var C = window.COMPARE;
    tests.push(eq('COMPARE 模块挂载', typeof C, 'object'));
    ...
    tests.push(eq('预置档案可取（杨禹赫/彭子旭）', (a1 && a2) ? 'Y' : 'N', 'Y'));
    var savedCmp = localStorage.getItem('bz_cmp_state');
    C._setState([{ type:'arch', id:a1.id }, { type:'arch', id:a2.id }]);
    C.open();
    ...
    tests.push(eq('卡1=杨禹赫', cards[0].getAttribute('data-name'), ARCHIVE.getDisplayName(a1)));
    tests.push(eq('卡2=彭子旭', cards[1].getAttribute('data-name'), ARCHIVE.getDisplayName(a2)));
    var bc = cards[0].querySelector('.body-cols');
    tests.push(eq('body-cols 带 luck-below', bc.classList.contains('luck-below') ? 'Y' : 'N', 'Y'));
    tests.push(eq('luck-below 生效为 block 布局（大运流年下移）', getComputedStyle(bc).display, 'block'));
    tests.push(eq('卡片无 top-bar（卡片头替代）', cards[0].querySelector('.top-bar') ? 'N' : 'Y', 'Y'));
    var luckCol = bc.querySelector('.luck-col');
    tests.push(eq('大运流年区存在（含 luck-section）', (luckCol && luckCol.querySelector('.luck-section')) ? 'Y' : 'N', 'Y'));
    tests.push(eq('大运流年区在三垣表下方（DOM序）', luckCol.previousElementSibling.classList.contains('main-col') ? 'Y' : 'N', 'Y'));
    var ths = cards[0].querySelectorAll('table.chart tr.hd th');
    tests.push(eq('表头7列：时柱右侧大运/流年列保留', ths[5].textContent + '/' + ths[6].textContent, '大运/流年'));
    var dyTd = cards[0].querySelector('td.col-dy');
    tests.push(eq('当前大运列有值', (dyTd && dyTd.textContent.trim().length > 0) ? 'Y' : 'N', 'Y'));
    var lnTd = cards[0].querySelector('td.col-ln');
    tests.push(eq('当前流年列有值', (lnTd && lnTd.textContent.trim().length > 0) ? 'Y' : 'N', 'Y'));
    var ts = getComputedStyle(track);
    tests.push(eq('cmp-track 横向可滚动', (ts.overflowX === 'auto' || ts.overflowX === 'scroll') ? 'Y' : 'N', 'Y'));
    tests.push(eq('拖拽手柄存在', cards[0].querySelector('.cmp-drag-handle') ? 'Y' : 'N', 'Y'));
    C.move(1, 0);
    var cards2 = track.querySelectorAll('.cmp-card');
    tests.push(eq('move(1,0) 后卡1=彭子旭', cards2[0].getAttribute('data-name'), ARCHIVE.getDisplayName(a2)));
    var stNow = localStorage.getItem('bz_cmp_state') || '';
    tests.push(eq('排序持久化（bz_cmp_state 含彭子旭id）', stNow.indexOf(String(a2.id)) >= 0 ? 'Y' : 'N', 'Y'));
    C.removeAt(0);
    tests.push(eq('removeAt 后剩一卡', String(track.querySelectorAll('.cmp-card').length), '1'));
    tests.push(eq('剩余卡=杨禹赫', track.querySelector('.cmp-card').getAttribute('data-name'), ARCHIVE.getDisplayName(a1)));
    C.clear(); C.close();
    if (savedCmp === null) localStorage.removeItem('bz_cmp_state'); else localStorage.setItem('bz_cmp_state', savedCmp);
    tests.push(eq('测试后状态还原', localStorage.getItem('bz_cmp_state'), savedCmp));
  })();

  // ===== 盘面对比·档案勾选与工具栏(v0.38.0) =====
  tests.push({ section:'盘面对比·档案勾选与工具栏(v0.38.0)' });
  (function() {
    var C = window.COMPARE;
    tests.push(eq('主操作行排盘对比入口已移除', document.getElementById('btnCompare') ? 'N' : 'Y', 'Y'));
    tests.push(eq('档案面板对比入口存在', document.getElementById('btnArchiveCompare') ? 'Y' : 'N', 'Y'));
    var ars = ARCHIVE.getArchives();
    var a1 = null;
    for (var i = 0; i < ars.length; i++) { if (ars[i].name === '杨禹赫') { a1 = ars[i]; break; } }
    tests.push(eq('预置档案可取', a1 ? 'Y' : 'N', 'Y'));
    var savedCmp = localStorage.getItem('bz_cmp_state');
    var savedZoom = localStorage.getItem('bz_cmp_zoom');
    localStorage.removeItem('bz_cmp_state');
    ARCHIVE.renderArchiveModal();
    var cb = document.querySelector('.archive-modal-row .archive-row-cmp[data-id="' + a1.id + '"]');
    tests.push(eq('档案行含对比勾选框', cb ? 'Y' : 'N', 'Y'));
    tests.push(eq('勾选框初始未选', cb.checked, false));
    cb.checked = true;
    cb.dispatchEvent(new Event('change'));
    tests.push(eq('勾选写入 bz_cmp_state', (localStorage.getItem('bz_cmp_state') || '').indexOf(String(a1.id)) >= 0 ? 'Y' : 'N', 'Y'));
    tests.push(eq('isPicked 反映勾选', C.isPicked('arch', a1.id) ? 'Y' : 'N', 'Y'));
    ARCHIVE.renderArchiveModal();
    cb = document.querySelector('.archive-modal-row .archive-row-cmp[data-id="' + a1.id + '"]');
    tests.push(eq('重绘后勾选态保持', cb.checked, true));
    cb.checked = false;
    cb.dispatchEvent(new Event('change'));
    tests.push(eq('取消勾选移除状态', C.isPicked('arch', a1.id), false));
    C._setState([{ type:'arch', id:a1.id }]);
    C.open();
    var bar = document.getElementById('cmpBar');
    tests.push(eq('cmpBar 名字条存在', bar ? 'Y' : 'N', 'Y'));
    var chips = bar.querySelectorAll('.cmp-chip');
    tests.push(eq('名字条 chip 数=卡数', chips.length + '/' + document.querySelectorAll('#cmpTrack .cmp-card').length, '1/1'));
    tests.push(eq('chip 文本含显示名', chips[0].textContent.indexOf(ARCHIVE.getDisplayName(a1)) >= 0 ? 'Y' : 'N', 'Y'));
    C.addCurrent();
    chips = bar.querySelectorAll('.cmp-chip');
    tests.push(eq('加当前盘后 chip 数=2', String(chips.length), '2'));
    tests.push(eq('chip 序与卡序一致', chips[0].getAttribute('data-key') + '|' + document.querySelector('#cmpTrack .cmp-card').getAttribute('data-key'), 'arch:' + a1.id + '|arch:' + a1.id));
    C.move(0, 1);
    chips = bar.querySelectorAll('.cmp-chip');
    tests.push(eq('move(0,1) 后 chip 序翻转', chips[0].getAttribute('data-key'), 'current'));
    tests.push(eq('move 后卡序同步翻转', document.querySelector('#cmpTrack .cmp-card').getAttribute('data-key'), 'current'));
    var sil = document.querySelector('#cmpTrack .cmp-card .cmp-card-siling');
    tests.push(eq('对比卡人元司令行存在', sil ? 'Y' : 'N', 'Y'));
    tests.push(eq('司令文本与算法一致', sil.textContent, ALGO.renYuanSiLing(a1.year, a1.month, a1.day, a1.hour, a1.min || 0)));
    var headEl = sil.closest('.cmp-card-head');
    tests.push(eq('司令并入生辰一行', headEl && headEl.querySelector('.cmp-card-meta').nextElementSibling === sil && sil.nextElementSibling.classList.contains('cmp-card-close') ? 'Y' : 'N', 'Y'));
    var lvBtn = document.getElementById('btnCmpLevel');
    tests.push(eq('级别按钮存在', lvBtn ? 'Y' : 'N', 'Y'));
    tests.push(eq('按钮初始文本=简分：少', lvBtn.textContent, '简分：少'));
    lvBtn.click();
    var lvPop = document.getElementById('cmpLevelPop');
    ...
    tests.push(eq('五区并排可勾', String(secCbs.length), '5'));
    tests.push(eq('五区顺序=运流项藏气三垣运流节气', String(secCbs.map(function(i){ return i.getAttribute('data-sec'); })), String(['yunxiang','cangqi','sanyuan','yunliu','jieqi'])));
    tests.push(eq('默认勾00100', String(secCbs.map(function(i){ return i.checked ? 1 : 0; }).join('')), '00100'));
    tests.push(eq('三垣区默认显示', (function(){ var r=document.querySelector('.cmp-card tr[data-sec~="sanyuan"]'); return r && getComputedStyle(r).display !== 'none' ? 'Y' : 'N'; })(), 'Y'));
    tests.push(eq('藏气行默认隐藏', (function(){ var r=document.querySelector('.cmp-card tr[data-sec~="cangqi"]'); return r && getComputedStyle(r).display === 'none' ? 'Y' : 'N'; })(), 'Y'));
    tests.push(eq('大运列表头默认隐藏', (function(){ var th=document.querySelector('.cmp-card th.col-dy'); return th && getComputedStyle(th).display === 'none' ? 'Y' : 'N'; })(), 'Y'));
    tests.push(eq('运流区默认隐藏', (function(){ var s0=document.querySelector('.cmp-card .luck-section'); return s0 && getComputedStyle(s0).display === 'none' ? 'Y' : 'N'; })(), 'Y'));
    tests.push(eq('起运行默认随运流区隐藏', (function(){ var h0=document.querySelector('.cmp-card .luck-head'); return h0 && getComputedStyle(h0).display === 'none' ? 'Y' : 'N'; })(), 'Y'));
    tests.push(eq('节气区默认隐藏', (function(){ var s0=document.querySelector('.cmp-card .jieqi-section'); return s0 && getComputedStyle(s0).display === 'none' ? 'Y' : 'N'; })(), 'Y'));
    tests.push(eq('卡宽随布局三等分', (function(){ var c0=document.querySelector('.cmp-card'); var track=document.getElementById('cmpTrack'); var z=(parseFloat(track.style.zoom)||100)/100; var n=parseInt(getComputedStyle(track).getPropertyValue('--cmp-cols'))||3; var w=c0.getBoundingClientRect().width/z; var want=(track.clientWidth-32-(n-1)*14)/n; return Math.abs(w-want)<3 ? 'Y' : 'N'; })(), 'Y'));
    secBar.querySelector('input[data-sec="yunxiang"]').click();
    tests.push(eq('勾回运流项后列显示且卡仍贴公式宽', (function(){ var th=document.querySelector('.cmp-card th.col-dy'); var c0=document.querySelector('.cmp-card'); var track=document.getElementById('cmpTrack'); var z=(parseFloat(track.style.zoom)||100)/100; var n=parseInt(getComputedStyle(track).getPropertyValue('--cmp-cols'))||3; var want=(track.clientWidth-32-(n-1)*14)/n; return th && getComputedStyle(th).display !== 'none' && Math.abs(c0.getBoundingClientRect().width/z-want)<3 ? 'Y' : 'N'; })(), 'Y'));
    secBar.querySelector('input[data-sec="yunxiang"]').click();
    tests.push(eq('再取消运流项后列隐藏卡仍贴公式宽', (function(){ var th=document.querySelector('.cmp-card th.col-dy'); var c0=document.querySelector('.cmp-card'); var track=document.getElementById('cmpTrack'); var z=(parseFloat(track.style.zoom)||100)/100; var n=parseInt(getComputedStyle(track).getPropertyValue('--cmp-cols'))||3; var want=(track.clientWidth-32-(n-1)*14)/n; return th && getComputedStyle(th).display === 'none' && Math.abs(c0.getBoundingClientRect().width/z-want)<3 ? 'Y' : 'N'; })(), 'Y'));
    secBar.querySelector('input[data-sec="cangqi"]').click();
    tests.push(eq('勾回藏气区后四柱藏气行显示', (function(){ var r=document.querySelector('.cmp-card tr.rh[data-sec~="cangqi"]'); return r && getComputedStyle(r).display !== 'none' ? 'Y' : 'N'; })(), 'Y'));
    secBar.querySelector('input[data-sec="sanyuan"]').click();
    tests.push(eq('取消三垣后行隐藏', (function(){ var r=document.querySelector('.cmp-card tr[data-sec~="sanyuan"]'); return r && getComputedStyle(r).display === 'none' ? 'Y' : 'N'; })(), 'Y'));
    secBar.querySelector('input[data-sec="sanyuan"]').click();
    tests.push(eq('勾回三垣后行显示', (function(){ var r=document.querySelector('.cmp-card tr[data-sec~="sanyuan"]'); return r && getComputedStyle(r).display !== 'none' ? 'Y' : 'N'; })(), 'Y'));
    secBar.querySelector('input[data-sec="cangqi"]').click();
    tests.push(eq('取消藏气区后四柱藏气行隐藏', (function(){ var r=document.querySelector('.cmp-card tr.rh[data-sec~="cangqi"]'); return r && getComputedStyle(r).display === 'none' ? 'Y' : 'N'; })(), 'Y'));
    secBar.querySelector('input[data-sec="sanyuan"]').click();
    secBar.querySelector('input[data-sec="sanyuan"]').click();
    ...
    tests.push(eq('勾回运流区后起运行显示', (function(){ var h1=document.querySelector('.cmp-card .luck-head'); return h1 && getComputedStyle(h1).display !== 'none' ? 'Y' : 'N'; })(), 'Y'));
    secBar.querySelector('input[data-sec="yunliu"]').click();
    tests.push(eq('再取消运流区后起运行隐藏', (function(){ var h2=document.querySelector('.cmp-card .luck-head'); return h2 && getComputedStyle(h2).display === 'none' ? 'Y' : 'N'; })(), 'Y'));
    secBar.querySelector('input[data-sec="cangqi"]').click();
    tests.push(eq('区显隐不影响档位文本', lvBtn.textContent, '简分：少'));
    secBar.querySelector('input[data-sec="jieqi"]').click();
    tests.push(eq('勾回节气区后块显示且档位文本不变', (function(){ var s2=document.querySelector('.cmp-card .jieqi-section'); return s2 && getComputedStyle(s2).display !== 'none' && lvBtn.textContent === '简分：少' ? 'Y' : 'N'; })(), 'Y'));
    secBar.querySelector('input[data-sec="jieqi"]').click();
    tests.push(eq('取消节气区后块隐藏', (function(){ var s1=document.querySelector('.cmp-card .jieqi-section'); return s1 && getComputedStyle(s1).display === 'none' ? 'Y' : 'N'; })(), 'Y'));
    tests.push(eq('五区恢复默认', String([].slice.call(secBar.querySelectorAll('input[data-sec]')).map(function(i){ return i.checked ? 1 : 0; }).join('')), '00100'));
    document.body.click();
    tests.push(eq('点面板外关闭面板', lvPop.classList.contains('open') ? 'N' : 'Y', 'Y'));
    tests.push(eq('缩放滑条存在', document.getElementById('cmpZoom') ? 'Y' : 'N', 'Y'));
    C.setZoom(120);
    tests.push(eq('setZoom 后缩放落在track', document.getElementById('cmpTrack').style.zoom, '120%'));
    tests.push(eq('缩放持久化', localStorage.getItem('bz_cmp_zoom'), '120'));
    tests.push(eq('百分数显示', document.getElementById('cmpZoomVal').textContent, '120%'));
    C.setZoom(100);
    tests.push(eq('对比页宫位面板已注入', document.querySelector('#cmpGzPanel .gz-popover') ? 'Y' : 'N', 'Y'));
    var gzBtn2 = document.querySelector('#cmpGzPanel .gz-trigger');
    gzBtn2.click();
    tests.push(eq('对比页宫位popover可开（就近定位）', document.querySelector('#cmpGzPanel .gz-popover').classList.contains('open') ? 'Y' : 'N', 'Y'));
    gzBtn2.click();
    tests.push(eq('再点收起', document.querySelector('#cmpGzPanel .gz-popover').classList.contains('open'), false));
    var removedOld = document.querySelector('#cmpOverlay [onclick*="togglePicker"]') || document.querySelector('#cmpOverlay [onclick*="addCurrent"]');
    tests.push(eq('添加盘/加入当前盘按钮已移除', removedOld ? 'N' : 'Y', 'Y'));
    var archBtn = document.querySelector('#cmpOverlay [onclick*="openArchivePanel"]');
    tests.push(eq('对比页档案按钮接入首页档案', archBtn && archBtn.textContent.indexOf('档案') >= 0 ? 'Y' : 'N', 'Y'));
    C.clear();
    var emptyTip = document.querySelector('#cmpTrack .cmp-empty');
    tests.push(eq('空态提示指向档案按钮', emptyTip && emptyTip.textContent.indexOf('📋 档案') >= 0 ? 'Y' : 'N', 'Y'));
    C.close();
    if (savedCmp === null) localStorage.removeItem('bz_cmp_state'); else localStorage.setItem('bz_cmp_state', savedCmp);
    if (savedZoom === null) localStorage.removeItem('bz_cmp_zoom'); else localStorage.setItem('bz_cmp_zoom', savedZoom);
    tests.push(eq('测试后状态还原', localStorage.getItem('bz_cmp_state') + '/' + localStorage.getItem('bz_cmp_zoom'), savedCmp + '/' + savedZoom));
  })();

  // ===== 对比页布局功能(v0.38.0) =====
  tests.push({ section:'对比页布局功能(v0.38.0)' });
  (function() {
    var C = window.COMPARE;
    var ars = ARCHIVE.getArchives();
    var a1 = null, a2 = null;
    ...
    var savedZoomL = localStorage.getItem('bz_cmp_zoom');
    localStorage.removeItem('bz_cmp_layout');
    var btns = document.querySelectorAll('.cmp-cols-btn');
    tests.push(eq('布局按钮组6档', String(btns.length), '6'));
    C.setCols(3);
    tests.push(eq('默认布局3档高亮', document.querySelector('.cmp-cols-btn.active') ? document.querySelector('.cmp-cols-btn.active').getAttribute('data-cols') : '', '3'));
    C._setState([{ type:'current' }, { type:'arch', id:a1.id }, { type:'arch', id:a2.id }]);
    C.open();
    var track = document.getElementById('cmpTrack');
    var bar = document.getElementById('cmpBar');
    var cs = track.querySelectorAll('.cmp-card');
    tests.push(eq('三卡渲染', String(cs.length), '3'));
    tests.push(eq('cols=3 三卡同一行', (cs[0].offsetTop === cs[1].offsetTop && cs[1].offsetTop === cs[2].offsetTop) ? 'Y' : 'N', 'Y'));
    C.setCols(2);
    cs = track.querySelectorAll('.cmp-card');
    tests.push(eq('cols=2 第三卡换行', cs[2].offsetTop > cs[0].offsetTop ? 'Y' : 'N', 'Y'));
    var zW = (parseFloat(document.getElementById('cmpTrack').style.zoom) || 100) / 100;
    var wantW = (track.clientWidth - 32 - 14) / 2;
    tests.push(eq('cols=2 卡宽对分', Math.abs(cs[0].getBoundingClientRect().width / zW - wantW) < 3 ? 'Y' : 'N', 'Y'));
    var chips = bar.querySelectorAll('.cmp-chip');
    tests.push(eq('cols=2 名字条第三枚换行', chips[2].offsetTop > chips[0].offsetTop ? 'Y' : 'N', 'Y'));
    var dt = new DataTransfer();
    var dragChip = chips[0], targetChip = chips[2];
    dragChip.dispatchEvent(new DragEvent('dragstart', { dataTransfer: dt, bubbles: true }));
    var r2 = targetChip.getBoundingClientRect();
    targetChip.dispatchEvent(new DragEvent('dragover', { dataTransfer: dt, bubbles: true, cancelable: true, clientX: r2.left + 2, clientY: r2.top + 2 }));
    var cardKeys = [].slice.call(track.querySelectorAll('.cmp-card')).map(function(c){ return c.getAttribute('data-key'); });
    tests.push(eq('拖chip后卡序同步', cardKeys.join(','), 'arch:' + a1.id + ',current,arch:' + a2.id));
    dragChip.dispatchEvent(new DragEvent('dragend', { dataTransfer: dt, bubbles: true }));
    var cs2 = track.querySelectorAll('.cmp-card');
    var dragCard = cs2[0], targetCard = cs2[2];
    dragCard.dispatchEvent(new DragEvent('dragstart', { dataTransfer: dt, bubbles: true }));
    var r3 = targetCard.getBoundingClientRect();
    targetCard.dispatchEvent(new DragEvent('dragover', { dataTransfer: dt, bubbles: true, cancelable: true, clientX: r3.left + r3.width - 2, clientY: r3.top + 2 }));
    var chipKeys = [].slice.call(bar.querySelectorAll('.cmp-chip')).map(function(c){ return c.getAttribute('data-key'); });
    tests.push(eq('拖卡后chip序同步', chipKeys.join(','), 'current,arch:' + a2.id + ',arch:' + a1.id));
    dragCard.dispatchEvent(new DragEvent('dragend', { dataTransfer: dt, bubbles: true }));
    C.setCols(5);
    tests.push(eq('布局持久化 bz_cmp_layout', localStorage.getItem('bz_cmp_layout'), '5'));
    C.setZoom(150);
    tests.push(eq('zoom150缩放落在track', document.getElementById('cmpTrack').style.zoom, '150%'));
    var zc = track.querySelectorAll('.cmp-card');
    tests.push(eq('zoom150行盘数不变', (zc[0].offsetTop === zc[1].offsetTop && zc[1].offsetTop === zc[2].offsetTop) ? 'Y' : 'N', 'Y'));
    C.setZoom(100);
    C.toggleBar();
    tests.push(eq('名字条开关收起', document.getElementById('cmpBar').style.display, 'none'));
    C.toggleBar();
    tests.push(eq('名字条开关展开恢复', document.getElementById('cmpBar').style.display, ''));
    tests.push(eq('名字条开关持久化', localStorage.getItem('bz_cmp_bar'), '1'));
    C.clear(); C.close();
    ...
    if (savedZoomL === null) localStorage.removeItem('bz_cmp_zoom'); else localStorage.setItem('bz_cmp_zoom', savedZoomL);
    if (savedCmp === null) localStorage.removeItem('bz_cmp_state'); else localStorage.setItem('bz_cmp_state', savedCmp);
  })();
