...
- **简分级别**：首页与对比页工具栏各有一个「简分」按钮（宫位式点击下拉面板 `cmpLevelPop`），按钮文本实时显示当前档位（「简分：少/简/中/详/全」或自由组合时「简分：自定义」）。面板上半部五档按钮并排（少=全部隐藏、简=星运+自坐、中=+纳音纳运星曜、详=+空亡、全=+神煞），中部七项横排勾选（星曜/纳音/纳运/星运/自坐/空亡/神煞，checkbox 用户直接勾）——点档位=按预设勾、手动勾=自定义状态、再点档位回预设档文本。**全文档一个状态**（`_levelRows` 七布尔 + `LEVEL_PRESETS`）：首页排盘表与对比卡内 `table.chart` 的行显隐实时同步（`applyLevelRows` 按 `data-row-type` 设行内联 display），两个按钮文本同步；点面板外自动关闭。档位不持久化，刷新回「少」。
- **五区显隐勾选栏**：对比页工具栏下方常驻一行五区勾选（运流项/藏气区/三垣区/运流区/节气区，`#cmpSecBar` + `_cmpSecs` 五布尔），**不放进简分面板、直接展示在对比页**，仅作用于对比卡（`.cmp-card` 加 `sec-*-off` 类 + CSS `!important`，与行级简分勾选分层互不打架）。默认勾 00100（**默认只勾三垣区**，其余四区隐藏——打开对比页即窄卡三盘布局）。五区定义——**运流项**=时柱右侧大运/流年两列（renderChart 表头大运 th 补 `col-dy` 类，勾选藏 `.col-dy`/`.col-ln` 全部单元格）；**藏气区**=四柱表「藏气」行（mainRows[3] 打 `data-sec="cangqi"`）；**三垣区**=卡内 `data-sec="sanyuan"` 行段（宫位标签行按 `gz-tags-sy` 一并隐藏；三垣区内藏气行打 `data-sec="sanyuan cangqi"` 双标记，CSS 选择器用 `~=` 多值匹配——藏气区隐藏时即便勾回三垣区，三垣藏气行仍被压住不出现）；**运流区**=下方大运流年块 `.luck-section` 连同其上方起运/交运一行（`.luck-head`，含 📍 今年按钮，v0.38.0 内并入运流区显隐）；**节气区**=`.jieqi-section`。**卡宽随布局公式**（v0.38.0 内被布局功能取代，见下条）：运流项显隐不再联动卡宽，`.luck-section` 加 `contain: inline-size` 不参与撑宽、自身 100% 横滚，卡加 `overflow-x:auto` 兜底，并放开 `.chart` 的 `min-width:680px`（窄卡内表格 100% 贴合不溢出，实测 476px 卡表格 450px 无溢出）。区勾选不参与「简分：X」档位文本判断，`applyCmpSecs` 反向同步勾选栏 checked，不持久化，刷新回默认。
- **对比页布局功能**：工具栏「布局」按钮组一行 1–6 盘可选（`COMPARE.setCols` + `bz_cmp_layout` 持久化，默认 3），`.cmp-track` 改 flex-wrap，卡宽公式 `calc((100% - (n-1)*14px)/n)`；名字条 chip 同公式等分，与卡逐行对应（一行三盘时名字第 4 枚落在第二行、与第四盘对齐）；拖动同步——拖名字 chip 或拖卡时 DOM 实时镜像搬动（`_cmpPairMove`：chip dragover 即时搬卡、卡 dragover 即时搬 chip，dragend 统一落盘重渲），名字与盘面同步在动，顺序与实际行位置一致；同时移除原 `.sec-yunxiang-off` 的 clamp 自动压缩宽，运流项显隐与卡宽解耦。**缩放与布局解耦**：缩放 zoom 从每张卡改设在 `.cmp-track` 上（卡布局宽不随 zoom 放大，任一缩放档位下行盘数恒定，整体渲染放大、track 自身横滚）；**卡贴公式不撑破**：`.cmp-card` 加 `min-width:0; overflow-x:auto`，内容再宽（如勾运流项后表格 680px）也压不破 n 等分槽位，实盘严格按所选列数分行（内容在卡内/运流区横滚）；**名字条可折叠**：布局按钮组后新增「名字」开关（`COMPARE.toggleBar` + `bz_cmp_bar` 持久化，默认开），收起名字条把上方空间让给盘面（1512×771 实测省 72px 纵向）。
- **藏气行高统一**：藏干 1–3 个不等导致藏气行高低错落（1 藏干柱 ≈65px vs 3 藏干柱 ≈92px），跨卡、同卡四柱行与三垣行之间不齐。CSS `.cmp-card tr[data-sec~="cangqi"] td { height:calc(4.8em + 11px); }` 统一为三藏干满高（17px 字号下 ≈92.6px，em 随字号自适应；+11px 覆盖 border-collapse 满行自然高 92.1px 的子像素），选择器 `~="cangqi"` 同时命中四柱藏气行与三垣藏气行。实测三卡（含单藏干柱盘）6 行全部 92.59px 零差异。断言 +2：四柱三垣藏气行同高（差 <1px 且满高）、td 高度 = calc 公式值（±2.5px）。
...
- **测试**：?test=1「盘面对比·档案勾选与工具栏(v0.38.0)」62 条断言（入口迁移/勾选写入与取消/isPicked/重绘保持/chip 数与序/chip 卡序一致/move 联动/司令行与算法值一致/司令并入生辰行且位于生辰与移除之间/添加盘与加入当前盘移除/档案按钮接入/空态提示指向档案/简分按钮初始文本/面板开合/五档并排/七项横排可勾/初始全不勾/手动勾行显示且主表同步/自由组合文本自定义/点详勾 6 项空亡显示神煞隐藏/文本回档位/setLevel(0) 回少/五区勾选栏常驻/五区并排/五区顺序/默认勾 00100/三垣行默认显示/藏气行默认隐藏/大运列表头默认隐藏/运流区默认隐藏/节气区默认隐藏/卡宽随布局公式宽/勾回运流项后列显示且卡仍贴公式宽/再取消运流项后列隐藏卡仍贴公式宽/勾回藏气区四柱藏气行显示/取消三垣行隐藏/勾回三垣行显示/取消藏气区四柱藏气行隐藏/藏气区隐藏时勾三垣三垣藏气行仍隐藏/藏气区勾回后三垣藏气行显示/区显隐不影响档位文本/勾回节气区块显示/取消节气区块隐藏/五区恢复默认00100/外点关闭/zoom 应用与持久化/第二实例 popover 开合/状态还原/藏气行高统一四柱三垣同高/藏气行满高calc公式/起运行默认随运流区隐藏/勾回运流区后起运行显示/再取消运流区后起运行隐藏/布局按钮组6档/默认布局3档高亮/三卡渲染/cols=3 三卡同一行/cols=2 第三卡换行/cols=2 卡宽对分/cols=2 名字条第三枚换行/拖chip后卡序同步/拖卡后chip序同步/布局持久化 bz_cmp_layout/zoom150缩放落在track/zoom150行盘数不变/名字条开关收起/名字条开关展开恢复/名字条开关持久化），总计 876 条全绿（v0.30 T12 星曜设置面板窗口高度 2 条环境项在部分环境可能常驻失败，与本节无关）；
- **实测**：真实浏览器——档案面板勾选杨禹赫+彭子旭（32 行勾选框可见）→「▦ 盘面对比」进对比页 → 名字条完整 DataTransfer 拖拽链拖 chip 翻转（chips、卡片 DOM、localStorage 三处同步）→ 级别按钮点至 level-1/level-4 回 level-0 → 宫位勾「脑区」标签行入卡 → 缩放 120%/115% 生效 → 司令行数值与 REN_YUAN 表核对（2021-06-01 立夏后 27 日→丙 ✓）。补测（按钮替换后）：对比页点「📋 档案」→ 面板以 z=1001 弹出于对比页之上（32 行可见）→ 勾选「邦顺」行 state 2 条、卡片实时变 2 张，取消后还原 1 条 1 卡；工具栏按钮序列核验（档案/简分/宫位/全选/清空/缩放/清空/关闭）。补测（简分可勾设置面板）：点「简分」→ 面板弹出（五档按钮并排、七项横排 checkbox、初始全不勾、按钮文本「简分：少」）→ 勾「星运」→ 主页表与对比卡星运行同步显示、文本变「简分：自定义」→ 点「详」→ 六项勾、空亡显示神煞隐藏、文本「简分：详」→ 点「少」→ 七行全隐、文本回「简分：少」→ 点面板外关闭。补测（五区显隐勾选栏）：对比页打开即见工具栏下方常驻五勾选（运流项/藏气区/三垣区/运流区/节气区，默认勾 00100 仅三垣区）→ 默认三垣行显示、其余区隐藏、卡宽 476px（1512 屏三盘并排正好一屏）→ 勾回「运流项」→ 大运/流年列出现、卡宽恢复 800px → 再取消回 476px → 勾回「藏气区」四柱藏气行显示 → 取消「三垣区」行隐藏再勾回显示 → 取消「藏气区」→ 勾「三垣区」三垣藏气行不复活 → 勾回「节气区」块显示再取消隐藏、档位文本全程不变、窄卡内表格 100% 贴合无溢出。现场已还原：宫位勾选恢复原状（认知·场景·客户）、对比页已关闭，`bz_cmp_state` 保留曾国藩勾选。
- **门禁**：node --check 四模块全过；check-release.sh 6/6；index.html ≡ standalone.html（md5 3ca6d04cc9f6af09a9cf61a7f9926821，1,234,408 B）。
...
- **实现**：render.js 新增 `applyTaiNianColumn(root)`——从三垣表头（`tr.hd` 首格文本以「三垣」起始）起向后遍历兄弟行，凡首格含 `rl` 类的行将其 cells[1] 按开关设显隐。同表歧义规避：盘式表头与三垣表头同处一张 table、且四柱行首格同为 `rl`，整表扫描会误隐年柱列，故必须以三垣表头为起点向后走。
- **挂载**：renderChart / 同卵双胎 / 龙凤双胎 / renderExpandedChart 四个渲染出口 + toggleShowTaiNian 内（gongwei.js 与 index.html/standalone.html 内联副本同步）。
- **修复**：applyTaiNianColumn 首版按「table 内首个 tr.hd 是否三垣表头」识别整表——因 chartRows 与 syRows 拼入同一张 table（首个 hd 是盘式表头）导致功能整体失效，已改为 tr.hd 全量扫描+兄弟遍历。
- **测试**：?test=1 新增 TN07b（开·表头胎年可见+文本）/TN08b（关·表头与干支行隐藏、胎元列与年柱表头不动）/TN10（清键重渲默认隐藏）/TN11（开·零重排恢复），共 767 条；TN 组全绿，仅余 2 条已知 T12 窗口高度环境性失败（与版本无关）。
- **实测**：真实浏览器点击——默认排盘胎年柱全隐藏 → 勾「显示胎年」全列恢复 + localStorage `bz_gongwei_show_tainian='1'` → 取消全列隐藏 + `'0'` → 四柱区不受影响。

---

## v0.36.0 — 三垣区新增「胎年」：胎元所在年干支入盘 (2026-09-20)

### 新增
- **三垣区新增「胎年」一柱**：胎年 = 胎元所在年的干支（口径A，邦顺裁决）。计算公式：`k = 月支序（寅=0…丑=11）`，`胎年 = yearPillar(effYear + floor((k-9)/12))`，其中 9 = 胎元在 60 节气月环上相对出生月的偏移（月干进一、月支进三 ⇔ 往前第 9 个节气月）。亥/子/丑月生人胎年 = 生年干支（与年柱相同），其余生月胎年 = 前一年干支
- **胎年宫位显示开关**：宫位面板新增「胎年宫位」勾选，控制三垣标签行胎年格是否显示宫位体系词，**默认不显示（留空）**；`GW_INDEX` 扩展第 8 槽 `taiNian:7`，宫位编辑器第 8 输入框（gzEditL7，可留空）已贯通保存链路。各宫位组的胎年体系词待邦顺后续给词，留空时勾选也不渲染 undefined。开关为本地显示偏好（localStorage `bz_gongwei_show_tainian`），不入导出/云同步载荷
- **口径说明**：「胎元所在年」与「出生月往前 10 个月所在年」两口径仅在**亥月**分叉（亥月的胎元落在生年正月 vs 往前第 10 个月落在前一年丑月），经 1900–2100 全月逐点双实现交叉验证，其余 11 个生月两口径完全一致

### 变更
- `algorithm.js` 新增 `taiNianGZ(effYear, yueZhi)` 并入 `paipan()` 返回（键名 `taiNian`）；`api/handler.rb` 新增 `self.tai_nian`（Ruby 侧同公式）并输出到 `/paipan` 的 `pillars`；`panels/paipan/view.js` 三垣块表头与 4 行加胎年列
- 三垣区表格 7 列结构不变：胎年占用原「年柱下的空列」，四柱区零改动；`render.js` 三垣各行空列替换为胎年单元格，双胞胎卡片（`buildCardHTML`）三垣表头同步加「胎年」
- **宫位标签不动**：胎年占用空列后，`updateGongWeiTags` 按 th 文本定位的三垣标签行照旧对齐

### 验证
- 双实现交叉验证（1900–2100 全节气月）：真实引擎 2399 样本 vs 纯干支环独立实现 2412 样本，0 分歧；8 个锚点案例（含 1984 亥月→甲子、1990 亥月→庚午两口径分叉点）全过
- `?test=1`：**759 条断言全绿**（v0.35.0 基线 726 + 胎年 22 + 宫位开关 TN01-TN09 共 11 条：GW_INDEX 槽位、缺省不显示、编辑器第 8 槽存在、开关关/开渲染、留空无 undefined、真实 DOM 显隐、编辑器保存贯通）
- 浏览器 DOM 实测（1984-11-15 12:00）：胎年列 x=161/w=99 与年柱列完全重合，胎元/命宫/身宫列 x 坐标与月/日/时柱逐一重合，页面横向溢出 0；胎年=甲子（=年柱）、胎元=丙寅 ✓
- 宫位开关浏览器实测（真实 click 路径）：默认未勾、胎年格空；点选后格显组标签第 8 槽词、`bz_gongwei_show_tainian='1'`，再点回空、`'0'`；popover 含「胎年宫位 + 标签可留空 · 默认不显示」；编辑器 8 槽装载顺序 年=地/月=玄/日=黄/时=宙/胎元=天/胎年=荒/命宫=宇/身宫=洪 与 `GW_INDEX` 一致；测试现场已清理（组删/勾选常用恢复/键移除）
- 龙凤胎双卡实测（inTwin=2）：两张卡片三垣表头均为「三垣|胎年|胎元|命宫|身宫」，干支行「甲|丙|丙|庚」✓
- 门禁：五核心模块三端一致 ✓；7 小模块外部 vs 内联一致 ✓；节气真值 24 边界点 ✓；`index.html` ≡ `standalone.html`（md5 `73a5ab4e8acc41f0cee48569291cee87`，1,185,312 字节）

---

## v0.35.0 — 人元司令边界修正：节后第 10 日起正确换丁 (2026-09-20)

### 修复
- **人元司令每段多占 1 日（P0 正确性）**：`REN_YUAN` 表注释口径为「辛司令节后第 1-9 日、丁第 10-12 日、戊第 13-30 日」，但 `renYuanSiLing()` 里 `daysAfter` 是 0 起算的完整天数（节后第 N 日 ⇔ daysAfter = N-1），判断却写 `daysAfter <= maxDay`，等价于每段多占 1 日 —— 丁的第一天（节后第 10 日）被辛吞掉。锚点案例：**1982-10-18 卯时**（寒露 1982-10-08 23:02 之后 9 天 7 小时，daysAfter = 9，即节后第 10 日）本应丁司令、排成辛。现改为 `daysAfter < maxDay`，与表口径一致

### 验证
- `?test=1` 新增 6 条司令边界断言（SL：戌月辛→丁翻转分钟 10-17 23:02/23:03、丁→戊 10-20 23:02/23:03、邦顺 1982-10-18 卯时→丁、含「寒露」）：**725 条全绿**（v0.34.0 基线 719 + 6）
- node 全表扫描：1982/2000/2024 三年 × 12 月 × 全部司令段边界，翻转分钟前后干支逐一核对，全部符合表口径
- 门禁：`scripts/check-release.sh` 全绿（三端模块一致 + 节气真值门禁 24 个换月边界点）
- 浏览器实测（file:// 真实排盘路径）：1982-10-18 06:00 → 盘头「人元司令：丁（寒露后 9 日）」
