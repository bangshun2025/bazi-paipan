...
## v0.13.1 — 宫位自定义 (2026-08-06)

### 新增
- **宫位自定义系统**：从「只读预置」升级为「完全可定制」— 新增/编辑/删除/排序/恢复
- **宫位设置面板**：模态弹窗，从 popover 底部「⚙ 宫位设置」按钮进入
- **14 种预置宫位组**：首屏自动初始化，含信息/认知/功能/做功/亲缘/兴趣/场景/人设/客户/系统/关系/圈层/生理/记忆
- **自定义宫位组**：支持创建新宫位组，组名+七柱标签各可自定义（≤12字）
- **拖拽排序 + 上下箭头**：宫位组顺序可调，首位↑置灰、末位↓置灰
- **删除进回收站**：软删除机制，支持单条恢复和清空回收站
- **重名校验**：新增/编辑时自动校验重名，UI 提示「该名称已存在」
- **导入/导出**：宫位组数据集 JSON 导入导出
- **localStorage 持久化**：宫位组配置 + 选中状态刷新不丢失
- **清除自动初始化**：localStorage 清除后自动恢复 14 组预置

### 修复 (v0.13.1)
- **Bug #1**：`persistGroups(groups)` 缺少 `gongWeiGroups = groups` 赋值 → 刷新后丢失自定义（阻塞）
- **Bug #2**：`errEl.style.display = ''` 无法覆盖 CSS `display:none` → 改为 `= 'block'`（中等）
- **Bug #3**：连锁 Bug（由 Bug #1 触发）→ 自动修复

### 变更量
- 前端：standalone.html（宫位自定义完整模块，~500+ 行增量）

### 测试
- QA 回归测试：29/31 通过，0 失败，1 条件通过（AC13 回收站 UI 受 confirm 阻塞）
